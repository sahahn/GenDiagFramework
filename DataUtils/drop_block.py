...
class DropBlock3D(keras.layers.Layer):
    """Based on https://arxiv.org/pdf/1810.12890.pdf"""
    ...
    def __init__(self,
        ...
        self.sync_channels = sync_channels
        self.data_format = K.normalize_data_format(data_format)
        self.input_spec = keras.engine.base_layer.InputSpec(ndim=5)
        self.supports_masking = True
    ...
    def _get_gamma(self, height, width, depth):
        """Get the number of activation units to drop"""
        height, width, depth = K.cast(height, K.floatx()), K.cast(width, K.floatx()), K.cast(depth, K.floatx())
        block_size = K.constant(self.block_size, dtype=K.floatx())
        return ((1.0 - self.keep_prob) / (block_size ** 3)) *\
               (height * width * depth / ((height - block_size + 1.0) *\
               (width - block_size + 1.0) * (depth - block_size + 1.0)))
